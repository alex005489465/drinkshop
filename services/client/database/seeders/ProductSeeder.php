...
class ProductSeeder extends Seeder
{
    ...
    private function assignProductsToCategories(): void
    {
        ...
        foreach ($categories as $category) {
            ...
            $category->update([
                'product' => json_encode($categoryProducts)
            ]);
        }
    }
    ...
    {
        $products = Product::all();
        $ingredients = Ingredient::all()->pluck('ingredient_name')->toArray();
        ...
        foreach ($products as $product) {
            ...
            'allowed_ingredients' => json_encode(\Faker\Factory::create()->randomElements($ingredients, 3)), // 隨機生成允許配料，模擬 JSON 欄位
            ]);
        }
    }
}
